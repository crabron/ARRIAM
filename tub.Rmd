...
less <- readr::read_tsv("~/storage/tub/output_kegg_table.tsv") %>% 
  filter(logFC < 0) %>% 
  filter(P.Value < 0.05) %>% 
  mutate(sum = round(round(C1 + C2 + C3 + L1 + L2 + L3)/10000))

dplyr::slice(less,rep(1:nrow(less), less$sum)) %>% 
  select(ID) %>% 
  mutate(id = row_number()) %>% 
  relocate(id) %>% 
  write_tsv('~/storage/tub/ko_for_minpath_leaf', col_names = FALSE)
...
dplyr::slice(less,rep(1:nrow(less), less$sum)) %>% 
  ...
  mutate(id = row_number()) %>% 
  relocate(id) %>% 
  write_tsv('~/storage/tub/ko_for_minpath_straw', col_names = FALSE)
...
cca <- vegan::cca(otus.ps.vegan ~  
                    Resperation + Humus + Cellulose  + C_per + N_per + Weight_loss, data=metadata)
...
heat_cell <-  amp_heatmap(amp.mags,
            tax_show = 5,
            group_by = "Group",
            tax_aggregate = "OTU",
            normalise=FALSE,
...
[jira](https://crabron.atlassian.net/browse/TR-27?atlOrigin=eyJpIjoiNjQ0ZWE0MGNkMzMwNDMxNzgzMmQ4YmYzODJiMWE2NDciLCJwIjoiaiJ9)
...
mad %>% 
  select(-type) %>% 
  pivot_longer(-c("id", "phase", "substrate")) %>% 
  group_by(name) %>%
  # mutate(value = scale(value)) %>% 
  mutate(phase = factor(phase, levels = c("0", "early", "middle", "late"))) %>%
  mutate(type = ifelse(name %in% c("Humus", "pH", "respiration", "TN", "TOC", "weight_loss"), 
                       "Agrochemistry",
                       "Enzyme activity")) %>% 
  ggplot(aes(y = value, x = phase, group = substrate)) +
    # geom_line(aes(color = name),
    #         alpha = 1) +
    stat_summary(aes(y = value, color = substrate), 
                 fun.y=mean, 
                 geom="line", 
                 size= 1.2, 
                 alpha=0.8) +
...
meta <- ps.putrid.its@sam_data %>% data.frame()
chem <- ps.putrid@sam_data %>% data.frame() %>% rownames_to_column("ID") %>% select(5:17)
meta_its <- cbind(meta, chem)
ps.putrid.its@sam_data <- sam_data(meta_its)
ps.putrid.its@sam_data
ps.putrid.its
...
ps.ff.r <- rarefy_even_depth(ps.ff, rngseed = 1433) 

p1 <- plot_alpha_w_toc_2(ps.ff.r, group="Group", metric="Observed")
p2 <- plot_alpha_w_toc_2(ps.ff.r, group="Group", metric="InvSimpson")

p_alpha <- ggpubr::ggarrange(p1, p2, nrow = 1)
p_alpha
...
estimate_richness(ps.ff.r) %>% 
  rownames_to_column("ID") %>% 
  left_join(ps.ff.r@sam_data %>%  
    data.frame() %>%  
    rownames_to_column("ID") %>% 
    mutate(ID = gsub(ID, pattern="-", replacement="."))) %>% 
  filter(decomposition_level != "priming") %>% 
  select(c("Observed","InvSimpson","decomposition_level", "matherial")) %>% 
  group_by(matherial)
...
p.alpha <- estimate_richness(ps.ff.r) %>% 
  rownames_to_column("ID") %>% 
  left_join(ps.ff.r@sam_data %>%  
    data.frame() %>%  
    rownames_to_column("ID") %>% 
  ...
  # ) %>% 
  pivot_longer(c( "Observed", "InvSimpson")) %>% 
  mutate(name = factor(name, levels = c("Observed", "InvSimpson"))) %>% 
  ggplot(aes(y = value, x = decomposition_level, group = matherial)) +
    # geom_line(aes(color = name),
    #         alpha = 1) +
    stat_summary(aes(y = value, color = matherial), fun.y=mean, geom="line") +
    labs(x = "decomposition_level",
       y = "z-scaled value") +
  geom_point(aes(shape = matherial, color = matherial)) +
  labs(title = "A. Alpha diversity - 16S") + 
...
p.alpha.its <- estimate_richness(ps.putred.its.r) %>% 
  rownames_to_column("ID") %>% 
  left_join(ps.putred.its.r@sam_data %>%  
    data.frame() %>%  
    rownames_to_column("ID") %>% 
    mutate(ID = gsub(ID, pattern="-", replacement="."))) %>% 
  filter(decomposition_level != "priming") %>% 
...
ps.putred.r <- rarefy_even_depth(ps.putrid, rngseed = 1433) 
amp.r <- phyloseq_to_ampvis2(ps.putred.r)
ps.putred.r@sam_data


p.heat.ph <- amp_heatmap(amp.r,
            tax_show = 15,
            group_by = "Group",
            tax_aggregate = "Phylum",
            normalise=TRUE,
            plot_values_size = 4.5,
            showRemainingTaxa = TRUE, tax_class = "Pseudomonadota",
            color_vector = colorspace::lighten(viridis::magma(n = 3, 
                                                              begin = 0.8, 
                                                              end = 0.001, 
                                                              direction = 1, 
                                                              alpha = 0.8), 0.4)) +
            theme(axis.text.x = element_text(size=14, angle=45, hjust=0.4),
              axis.text.y = element_text(size=13))+
  ggtitle("<span style='font-size: 14pt;'>C. Phylum relative abundance</font>
          <span style='font-size: 14pt;'>16S </font>") +
...
ps.putred.its.r <- rarefy_even_depth(ps.putrid.its, rngseed = 1433) 
amp.its.r <- phyloseq_to_ampvis_its(ps.putred.its.r)


p.heat.its.ph <- amp_heatmap(amp.its.r,
            tax_show = 8,
            group_by = "Group",
            tax_aggregate = "Phylum",
            normalise=TRUE,
            ...
            theme(axis.text.x = element_text(size=14, angle=45, hjust=0.4),
              axis.text.y = element_text(size=13))+
  ggtitle("<span style='font-size: 14pt;'>ITS2</font>") +
  guides(
    y = "none",
    y.sec = "axis"
  ) +
...
viridis::magma(n = 2, begin = 1, alpha = 0.2, end = 0.001, direction = 1)
```
...
cowplot::ggdraw(cowplot::switch_axis_position(p.heat.its.ph + theme_gray(), axis = 'y'))
...
```{r, fig.height=10, fig.width=10.5}

p.up.row <- gridExtra::grid.arrange(p.alpha, p.alpha.its, p.cca, 
             ncol = 3, nrow = 2, 
             heights = c(1, 1),
             widths = c(1, 1, 1.5),
...
gridExtra::grid.arrange(p.up.row, p.heat.ph, p.heat.its.ph,
             ncol = 2, nrow = 2, 
             heights = c(1, 1), 
...
colorspace::lighten(viridis::magma(n = 25, 
                                   begin = 0.8,
                                   end = 0.001,
                                   direction = 1,
                                   alpha = 0.8), 
...
minpath <- read_excel("minpath.xlsx", .name_repair = "universal")
minpath
heatta_minpath <- minpath %>% 
  select(name, fam.found_S, fam.found_L) %>% 
  arrange() %>% 
  head(20) %>%
  column_to_rownames("name") %>% 
  dplyr::rename('straw' = 'fam.found_S', 'leaves' = 'fam.found_L') %>% 
  pheatmap::pheatmap(
         main = c('B.Changing metabolic pathways'),
         ...
         legend = FALSE,
         fontsize_number = 8,
         color = colorspace::lighten(viridis::magma(n = 100, 
                                                    begin = 0.9, 
                                                    end = 0.2, 
                                                    direction = 1, 
                                                    alpha = 0.8), 0.4),
...
h.all <- read_csv("heat_all.txt")
funct.id <- read_tsv("function_heatmap_form.tsv")
h.all
funct.id
h.all %>% 
  select(1:34) %>% 
...
left.join(h.all, funct.id)

# data.frame(values = seq(1:33),
...
ama.ama <- c("hue", ama) %>% 
  tidy_names()
ama.ama <- gsub(pattern='\\.\\.', replacement='\\.\\.\\.', ama.ama , perl = TRUE)
ama.ama.trunc <- ama.ama[2:34]
funct.id.trunc$name <- ama.ama.trunc
funct.id.trunc

h.all %>% 
  select(1:34) %>% 
  pivot_longer(-site_id) %>% 
  left_join(funct.id.trunc, by = c("name")) %>% 
  select(c("site_id", "value", "function_name", "long_function_name")) %>%
  ggplot(aes(x=site_id, y=long_function_name, fill = value)) + 
  geom_raster() + 
  facet_grid(rows= vars(function_name), scale = "free_y", space = "free")
...
heatta_cazy <- h.all %>% 
  ...
  pivot_longer(-site_id) %>% 
  left_join(funct.id.trunc, by = c("name")) %>% 
  select(c("site_id", "value", "function_name")) %>% 
  group_by(function_name, site_id) %>%
  summarise(rel_abundance = sum(value, na.rm = T)*100) %>% 
  pivot_wider(values_from = rel_abundance, names_from = site_id) %>% 
  column_to_rownames("function_name") %>%
  pheatmap::pheatmap(
         main = c('A. CAZy relative abundance'),
         display_numbers = TRUE,
         number_format =  "%.2f",
         number_color = "black", 
         cluster_rows = TRUE,
...
gridExtra::grid.arrange(heatta_cazy, heatta_minpath, 
             ncol = 2, nrow = 1, 
             widths = c(1, 1))
...
puls_sum %>% 
  ...
  # filter(Phylum %in% c('Bacteroidota')) %>% 
  mutate(Phylum = fct_infreq(as.factor(Phylum))) %>% 
  ggplot(aes(x = Genome_Size/(Completeness*0.01), y = n_celu, label=Genus)) +
  # ggplot(aes(x = Genome_Size, y = n_celu, label=bin)) + 
    # ggplot(aes(x = Genome_Size, y = n_celu, label=paste0(bin,' ' ,Genus, ' ', Species))) + 
  geom_point(aes(color=Hat)) +
  geom_text_repel(size=3, alpha=0.5) +
...
cell <- puls_sum %>% 
  ...
  # filter(Phylum %in% c('Bacteroidota')) %>% 
  mutate(Phylum = fct_infreq(as.factor(Phylum))) %>% 
  ggplot(aes(x = Genome_Size, y = n_celu, label=Genus)) +
  # ggplot(aes(x = Genome_Size, y = n_celu, label=bin)) + 
    # ggplot(aes(x = Genome_Size, y = n_celu, label=paste0(bin,' ' ,Genus, ' ', Species))) + 
  geom_point(aes(color=Hat)) +
  geom_text_repel(size=3, alpha=0.5) +
  ...
  scale_colour_viridis_d(option = "magma", 
      ...
      labs(x = "genome size",
       y = "complex polysacharide utilisation loci") +
  ggtitle("A. MAGs with complex PULs") +
  theme(legend.position = "none") +
  theme(plot.title = element_text(size = 14, face = "bold")) +
  theme(axis.text.x = element_text(size=13, angle=45, hjust=0.4),
              axis.text.y = element_text(size=13))
...
amp.mags <- ampvis2::amp_filter_taxa(amp, mg, normalise = TRUE) %>% 
  ampvis2::amp_filter_samples(Phase != "priming", normalise = FALSE)

amp.mags$tax[amp.mags$tax == "Paenibacillus"] <- "Pristimantibacillus"
...
heat_cell <-  amp_heatmap(amp.mags,
            ...
            theme(axis.text.x = element_text(size=13, angle=45, hjust=0.4),
              axis.text.y = element_text(size=13)) +
  ggtitle("B. Relative abundance of selected ASVs ") + 
  theme(plot.title = element_text(size = 14, face = "bold"))
...
gridExtra::grid.arrange(cell, heat_cell, 
             ncol = 2, nrow = 1, 
             widths = c(1, 1))
...
mag9 <- mafs3_mod[c("l2_10_Pristimantibacillus",
              "l1_2_Pristimantibacillus",
              "l3_2_Pristimantibacillus",
              "l3_16_Ohtaekwangia",
              "c1_37_Streptomyces",
              "c1_12_Chitinophaga", 
              "c2_18_Ohtaekwangia",
...
mags3 <- read_csv("heat_mags3.txt")

mags3 %>% 
  filter(mag_id %in% hat2)
