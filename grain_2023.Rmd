...
mad_fal <- mad %>% group_by(sample) %>%  mutate(across(where(is.numeric), ~replace_na(., median(., na.rm=TRUE))))
mad_fal

read_excel("grain_2023_meta.xlsx")
path <- "grain_2023_meta.xlsx"
...
mad <- mad %>% 
  select(-c("type"))
mad_gra <- mad %>% group_by(sample) %>%  mutate(across(where(is.numeric), ~replace_na(., median(., na.rm=TRUE))))
mad_gra
mad
num_id <- unlist(purrr::map(stringr::str_split(mad$ID, "\\-", 2), function(x) x[[2]])) 
num_id <- unlist(purrr::map(stringr::str_split(mad$ID, "\\-", 2), function(x) x[[2]])) 
num_id
mad_id_mod <- paste0("Ab.", mad$id)
mad['ID'] <- mad_id_mod
...
map <- data_frame(
  id = hue,
  grain = grepl('SEQ283', hue),
  fallow = grepl('SEQ284', hue),
) %>% 
...
ps.f@sam_data
ps.gra <- prune_samples(sample_data(ps.f)$grain == TRUE, ps.f)
ps.gra  <- prune_taxa(taxa_sums(ps.gra) > 0, ps.gra)
ps.gra
...
ps.gra@sam_data <-   ps.gra@sam_data %>%
  ...
  mutate(id = as.double(str_match(ID, "(?<=3\\.).+$"))) %>% 
  left_join(mad_gra, by = 'id') %>% 
  column_to_rownames('ID') %>% 
...
plot_rich_reads_samlenames_lm(ps.gra, group = "Group", label = "Group")  
...
p_a1 <-  plot_alpha_w_toc_mod(ps.gra, group="Group",  metric="Observed") 
p_a2 <- plot_alpha_w_toc_mod(ps.gra, group="Group", metric="Shannon") 
p_a3 <- plot_alpha_w_toc_mod(ps.gra, group="Group", metric="InvSimpson") 
...
amp_heatmap(amp.gra,
            ...
            tax_add = "Phylum",
            group_by = "Group",
            plot_values_size = 2, facet_by = "soil_type",
...
ps.gra.fie <- prune_samples(sample_data(ps.gra.r)$soil_type == 'field', ps.gra.r)
ps.gra.fie  <- prune_taxa(taxa_sums(ps.gra.fie) > 0, ps.gra.fie)

ps.gra.for <- prune_samples(sample_data(ps.gra.r)$soil_type == 'forest', ps.gra.r)
ps.gra.for  <- prune_taxa(taxa_sums(ps.gra.for) > 0, ps.gra.for)


beta_fal <- beta_custom_norm_PCoA_elli_w(ps.gra.fal, Group = 'Group', Color = 'wheat')
beta_fie <- beta_custom_norm_PCoA_elli_w(ps.gra.fie, Group = 'Group', Color = 'wheat')
beta_for <- beta_custom_norm_PCoA_elli_w(ps.gra.for, Group = 'Group', Color = 'wheat')

ggpubr::ggarrange(beta_for, beta_fal, beta_fie, ncol = 3)
...
amp.gra.f <- ampvis2::amp_filter_samples(amp.gra, soil_type %in% c('fallow', 'field'), normalise = TRUE)
amp_heatmap(amp.gra.f,
            tax_show = 100,
            tax_aggregate = "Genus",
            tax_add = "Phylum",
            group_by = "Group",
