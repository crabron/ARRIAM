...
```{r, message=FALSE,echo=TRUE,fig.height=8, fig.width=8, results=FALSE, fig.keep='all'}

beta_custom_norm_NMDS_elli <- function(ps, seed = 7888, normtype="vst", factor="Description"){
  require(phyloseq)
  require(ggplot2)
  require(ggpubr)
  require(DESeq2)
  ...
  #plotting
  p  <-  plot_ordination(ps.varstab, ordination.b, type="sample", color= factor, title="NMDS - Bray-Curtis", axes = c(1,2) ) + 
    theme_bw() + 
    theme(text = element_text(size = 10)) + 
    geom_point(size = 3) +
    geom_mark_ellipse(aes(group = "What", label = "What"), label.fontsize = 10, label.buffer = unit(2, "mm"), label.minwidth = unit(5, "mm"),con.cap = unit(0.1, "mm"))
...
d
noquote(d)
p.beta <- beta_custom_norm_NMDS_elli(ps.f, factor="What")
p.beta
...
ggarrange(f1, f2, f3, labels = c("Луговая, B/Q", "Чернозём, B/Q", "Луговая/Чернозём"),nrow = 1)

write.table(des.so.filt, file = "des_so_filt.tsv", sep= "\t", col.names = NA, quote=FALSE)
...
p <- ggplot(fdat_amazing %>% filter(Score %in% c("sites","biplot"))) + 
  ...
  geom_segment(data = fdat_amazing %>% dplyr::filter(Score == "biplot"), aes(x = 0, xend = CCA1, y = 0, yend = CCA2), alpha=0.8, color = "red", arrow = arrow(angle = 3)) +
  geom_text_repel(aes(x=CCA1, y=CCA2, label= Label, colour = phylum_pro), size=fdat_amazing$label_size) + 
  xlab(paste0(round(cca$CCA$eig[1], 3)*100, "% CCA1")) +
  ylab(paste0(round(cca$CCA$eig[2], 3)*100, "% CCA2")) +
  grids(linetype = "dashed") +
  geom_vline(xintercept = 0, size = 0.75, color = "#737373", alpha=0.5) +
...
GP <- ps.cc
GP 
GP.f <-  filter_taxa(GP, function(x) sum(x > 3) > (0.1*length(x)), TRUE)
GP <- GP.f
GP
...
glmmod
top.coords <- as.matrix(coefficients(glmmod, s=0.03463347))
...
top.coords
tc.names <- sapply(paste0("n",str_sub(top.coords, 2)), function(x) name.balance(tree, tax, x))
tc.names <- sapply(top.coords, function(x) name.balance(tree, tax, x))
tree$
paste0("n",str_sub(top.coords, 2))
as.data.frame(tc.names)
tc.names
votes <- name.balance(tree, tax, 'n1114', return.votes = c('up', 'down'))
votes
...
tc.colors <- c('#a6cee3', '#1f78b4', '#b2df8a', '#33a02c', '#fb9a99', '#a6cee3', '#a6cee9')
...
p <- annotate_balance(tree, 'n5', p=p, labels = c('n5+', 'n5-'),
                 offset.text=0.15, bar=FALSE)
p <- annotate_balance(tree, 'n20', p=p, labels = c('n20+', 'n20-'),
                 offset.text=0.15, bar=FALSE)
p <- annotate_balance(tree, 'n252', p=p, labels = c('n252+', 'n252-'),
                 offset.text=0.15, bar=FALSE)
p <- annotate_balance(tree, 'n543', p=p, labels = c('n543+', 'n543-'),
                 offset.text=0.15, bar=FALSE)
p <- annotate_balance(tree, 'n551', p=p, labels = c('n551+', 'n551-'),
                 offset.text=0.15, bar=FALSE)
annotate_balance(tree, 'n737', p=p, labels = c('n737+', 'n737-'),
                 offset.text=0.15, bar=FALSE)
...
p1 <- ggarrange(p.alpha.Cdt.oo, p.alpha.Cdt.sh,p.alpha.Cdt.is, p.alpha.Cdt.pd , 
                ...
                ncol = 4 ,label.x = 0.105, nrow = 2, labels = c("Чернозём", "", "", "", "Луговая"))
p1
...
```{r, fig.height=9, fig.width=9}
amp <- phyloseq_to_amp(ps.cc)
amp_venn(amp, group_by = "Quarry_or_Benchmark", normalise = TRUE, cut_a = 0.000001, cut_f=0.000001)
