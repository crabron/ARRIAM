...
ps.f.nc <- prune_samples(sample_data(ps.f)$Geo != c("Lab"), ps.f)
ps.f.nc  <- prune_taxa(taxa_sums(ps.f.nc) > 0, ps.f.nc)

ps@sam_data
ordination.b.p <- phyloseq::ordinate(ps.f.nc, "NMDS", "bray")
...
plot_ordination(ps.f.nc, ordination.b.p, type="sample", color="Type", title="NMDS - Bray-Curtis", axes = c(1,2) ) + 
    theme_bw() + 
    theme(text = element_text(size = 10), legend.position = "none") + 
    geom_point(size = 3) +
    geom_mark_ellipse(aes(group = Type, label = Type), label.fontsize = 10, label.buffer = unit(2, "mm"), label.minwidth = unit(5, "mm"),con.cap = unit(0.1, "mm"))
...
amp <- phyloseq_to_amp(ps.f.nc)

p_l <- amp_heatmap(amp,tax_show = 30, group_by = "Type", tax_aggregate = "Genus", tax_class = "Proteobacteria", tax_add = "Family") + theme_bw() + theme(text = element_text(size=15), legend.position = "none") + theme(axis.text.x=element_text(angle=45,hjust=1))

p_r <- amp_heatmap(amp,tax_show = 12, group_by = "Type", tax_aggregate = "Phylum", tax_class = "Proteobacteria") + theme_bw() + theme(text = element_text(size=15), legend.position = "none") + theme(axis.text.x=element_text(angle=45,hjust=1))


ggarrange(p_l, p_r, ncol = 2 , nrow = 1, common.legend = TRUE, legend = "none", font.label = list(size = 12, face = "bold", color ="black"))
```


```{r,message=FALSE,echo=TRUE, fig.height=5, fig.width=5.8}

dist <- phyloseq::distance(ps.f.nc, method = "bray")
groups <- ps.f.nc@sam_data$Type
...
plot_alpha(ps.f.nc,  c("Observed", "InvSimpson", "Shannon"), "Type") 
...
```{r}
alpha <- estimate_richness(ps.f.nc, measures=c("Observed", "InvSimpson", "Shannon"))

mod$distances

cor.test(alpha$Observed, mod$distances)
