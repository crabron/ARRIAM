...
=======
options(width = 1800)
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
```
...
setwd("~/storage/notebooks/")
=======
##Import data, some renaming
```{r, message=FALSE, rows.print = 40}
setwd("~/storage/plates/plos/")
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
library(phyloseq)
...
###Some ampwis2 visualisations
####Create ampwis2 object
...
amp_merged
p.differ
p.differ.1 <- p.differ+  theme(legend.position = "bottom")+ ggtitle("B. dependence of the abundance of the core phylotypes") +  xlab("median bulk soil vst count") + ylab("median Hutchinson culturome vst count") + theme(text = element_text(size = 10))
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
...
=======
ps.cz.soil <- prune_samples(sample_data(ps.cz)$Medium_or_Soil %in% c("soil"), ps.cz)
ps.cz.soil  <- prune_taxa(taxa_sums(ps.cz.soil ) > 0, ps.cz.soil )

ps.cz.medium <- prune_samples(sample_data(ps.cz)$Medium_or_Soil %in% c("medium"), ps.cz)
ps.cz.medium  <- prune_taxa(taxa_sums(ps.cz.medium ) > 0, ps.cz.medium )

ps.sp.soil <- prune_samples(sample_data(ps.sp)$Medium_or_Soil %in% c("soil"), ps.sp)
ps.sp.soil <- prune_taxa(taxa_sums(ps.sp.soil) > 0, ps.sp.soil)

ps.sp.medium <- prune_samples(sample_data(ps.sp)$Medium_or_Soil %in% c("medium"), ps.sp)
ps.sp.medium <- prune_taxa(taxa_sums(ps.sp.medium) > 0, ps.sp.medium)
...
inner_join(tax.d.sp.soil, tax.d.sp.medium)

tax.t.sp.soil <- as_tibble(ps.sp.soil@tax_table@.Data, rownames = "ID")
tax.t.sp.medium <- as_tibble(ps.sp.medium@tax_table@.Data, rownames = "ID")
tax.core.sp <- inner_join(tax.t.sp.medium, tax.t.sp.soil)

tax.t.cz.soil <- as_tibble(ps.cz.soil@tax_table@.Data, rownames = "ID")
tax.t.cz.medium <- as_tibble(ps.cz.medium@tax_table@.Data, rownames = "ID")
tax.core.cz <- inner_join(tax.t.cz.medium, tax.t.cz.soil)

tax.core <-  inner_join(tax.core.sp, tax.core.cz)
...
```{r}
amp.cz.soil <- phyloseq_to_amp(ps.cz.soil)
amp.ps.medium <- phyloseq_to_amp(ps.cz.medium)
amp.sp.soil <- phyloseq_to_amp(ps.sp.soil)
amp.sp.medium <- phyloseq_to_amp(ps.sp.medium)
amp.cz <- phyloseq_to_amp(ps.cz)
amp.sp <- phyloseq_to_amp(ps.sp)
amp.cz.soil
amp.ps.medium
...
amp_venn(amp.cz, group_by = "Medium_or_Soil", cut_a = 0.0000000000001, cut_f = 0.00000000000001)
amp_venn(amp.sp, group_by = "Medium_or_Soil", cut_a = 0.0000000000001, cut_f = 0.00000000000001)

```


##Varstab normalisation. little plot. New one.
```{r, fig.height=6, fig.width=8,  rows.print = 50}
...
meta <- as(sample_data(physeq), "data.frame")
otus <- t(otus)
deseq_counts <- DESeqDataSetFromMatrix(otus, colData = meta, design = ~ Description) 
deseq_counts_vst <- varianceStabilizingTransformation(deseq_counts)
vst_trans_count_tab <- assay(deseq_counts_vst)
vst_trans_count_tab %>% as.data.frame() %>% arrange(desc(kimeklis.K.10))
# vst_trans_count_tab[vst_trans_count_tab < 0.0] <- 0.0

aggregation_by_meta <- function(vst_trans_count_tab, meta, aggfunc){
  d.vst <- as.data.frame(vst_trans_count_tab)
  t.d.vst <- t(d.vst)
  vst.median <- aggregate(t.d.vst, by=list(meta$Description), aggfunc) 
  ...
  vst.median <- as.data.frame(t(vst.median))
  return(vst.median)
}

vst.median <- aggregation_by_meta(vst_trans_count_tab, meta, median) 
vst.mean <- aggregation_by_meta(vst_trans_count_tab, meta, mean)
```
...
###long format for ggplotting 

```{r}
     
vst.median.melted <- vst.median %>%
  filter((gSP > -6 & bsSP > -6) | (gCZ > -6 & bsCZ > -6)) %>%
  rownames_to_column("ID") %>% 
  reshape2::melt(variable.name = "Soil" ,value.name = "culturome" , measure.vars = c("gSP", "gCZ")) %>%
  reshape2::melt(variable.name = "Soil2" ,value.name = "bulk_soil" , measure.vars = c("bsSP", "bsCZ")) %>% 
  filter((Soil == "gSP" & Soil2 == "bsSP")|(Soil == "gCZ" & Soil2 == "bsCZ")) %>% 
  select(-c(Soil2)) %>%
  mutate(Soil = revalue(Soil, c("gSP" = "SP", "gCZ" = "CZ"))) %>% 
  filter(culturome > -6 & bulk_soil > -6)
vst.median.melted
p.differ <- ggplot(data=vst.median.melted) + geom_point(aes(x = bulk_soil, y = culturome, color = Soil))  + theme_bw()
p.differ

d.core.sp <- vst.median.melted [vst.median.melted$Soil == "SP",]
d.core.cz <- vst.median.melted [vst.median.melted$Soil == "CZ",]
d.core.sp 
d.core.cz
fisher.core(d.core.sp)
fisher.core(d.core.cz)
```
...
```{r}
d.lm <- d.core.sp
fisher.core <- function(d.lm){
  lm1 <- lm(culturome ~ d.lm[,4], data=d.lm)
  SS1 <- sum(lm1$residuals^2)
  lm0 <- lm(culturome~1, data=d.lm)
  SS0 <- sum(lm0$residuals^2)
  dof <- sum(d.lm$culturome>0)-1
  pval <- pf(SS1/SS0,dof,dof)
  return(paste0("pval by Fisher test for " , d.lm$Soil[1] , " - ", pval))
}
```

###fisher test. execution.

```{r}
    
vst.mean.melted <- vst.mean %>%
  filter((gSP > -6 & bsSP > -6) | (gCZ > -6 & bsCZ > -6)) %>%
  rownames_to_column("ID") %>% 
  reshape2::melt(variable.name = "Soil" ,value.name = "culturome" , measure.vars = c("gSP", "gCZ")) %>%
  reshape2::melt(variable.name = "Soil2" ,value.name = "bulk_soil" , measure.vars = c("bsSP", "bsCZ")) %>% 
  ...
  filter(culturome > -6 & bulk_soil > -6)
vst.mean.melted
p.differ <- ggplot(data=vst.mean.melted) + geom_point(aes(x = bulk_soil, y = culturome, color = Soil))  + theme_bw()
p.differ

d.core.sp <- vst.mean.melted[vst.mean.melted$Soil == "SP",]
d.core.cz <- vst.mean.melted[vst.mean.melted$Soil == "CZ",]
d.core.sp 
d.core.cz
fisher.core(d.core.sp)
fisher.core(d.core.cz)
...
```{r}
hist.median <- hist(vst.median, breaks = 0.5)
library(ggpubr)
ggarrange(ggdensity(vst.median$gSP), ggdensity(vst.median$gCZ),ggdensity(vst.median$bsSP),ggdensity(vst.median$bsCZ), ncol = 2, nrow = 2, labels = c("gSP","gCZ","bsSP","bsCZ"))
vst.mean.melted
ggplot(data = vst.mean.melted.all, aes(x = bulk_soil)) + geom_density(aes(color = Soil))
vst.mean.melted.all <- vst.mean %>%
  rownames_to_column("ID") %>% 
  reshape2::melt(variable.name = "Soil" ,value.name = "culturome" , measure.vars = c("gSP", "gCZ")) %>%
  reshape2::melt(variable.name = "Soil2" ,value.name = "bulk_soil" , measure.vars = c("bsSP", "bsCZ")) %>% 
  filter((Soil == "gSP" & Soil2 == "bsSP")|(Soil == "gCZ" & Soil2 == "bsCZ")) %>% 
...
```{r}
hist.mean <- hist(vst.mean, breaks = 0.5)
ggarrange(ggdensity(vst.mean$gSP), ggdensity(vst.mean$gCZ),ggdensity(vst.mean$bsSP),ggdensity(vst.mean$bsCZ), ncol = 2, nrow = 2, labels = c("gSP","gCZ","bsSP","bsCZ"))
...
```{r}
library(tidyverse)
taxa.all.tidy <- as.data.frame(ps.f@tax_table@.Data) %>% rownames_to_column("ID")
core.res.with.taxa <- left_join(vst.mean.melted, taxa.all.tidy, by ="ID")
core.res.with.taxa %>%  filter(Soil == "SP") %>%  group_by(Class) %>% dplyr::summarise(new = n_distinct(ID)) %>% arrange(desc(new))
core.res.with.taxa %>%  filter(Soil == "CZ")  %>% mutate(abund_mean = culturome + bulk_soil) %>% arrange(desc(abund_mean))  %>%  filter(Phylum == "Firmicutes") 



vst.median %>% rownames_to_column("ID") %>% 
  filter((gSP > -6 & bsSP < -6) | (gCZ > -6 & bsCZ < -6)) -> cult

core.cult.res.with.taxa <- left_join(core, taxa.all.tidy, by ="ID")

core.cult.res.with.taxa 
```

### group by taxanomy
```{r}
library(tidyverse)
core.res.with.taxa %>%  filter(Soil == "CZ") %>%  group_by(Class) %>% dplyr::summarise(new = n_distinct(ID)) %>% arrange(desc(new))
```

##Beta-diversity plots

```{r, message=FALSE,fig.height=5, fig.width=15, results=FALSE, fig.keep='all', dev ="tiff"}
beta_custom_norm_PCoA_elli <- function(ps, seed = 7888, normtype="vst", color="Repeats"){
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
  require(phyloseq)
  require(ggplot2)
  require(ggpubr)
...
=======
  ...
  ps.vst.compl <- physeq
  otu.table.compl[otu.table.compl < 0.0] <- 0.0
...
```{r message=FALSE}
=======
  return(list(p.all, ordination.b, ordination.u, ordination.w))
  }

beta_plot_function_result <- beta_custom_norm_PCoA_elli(ps.f)
beta_plot_function_result[[1]]
plot_scree(beta_plot_function_result[[3]])
ord_b <- beta_plot_function_result[2]
...
```{r echo=FALSE, message=FALSE, results=FALSE, fig.height=12, fig.width=16}
=======
###Tree for soil soil-specific taxa
...
###diff tables
####realy have no any idea what code below are doing
...
<<<<<<< HEAD
=======
-- Jacob, maybe your a nice person, but your code sucks
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
...
<<<<<<< HEAD
```{r echo=FALSE, message=FALSE, results=FALSE, fig.height=18, fig.width=16, eval=TRUE}
=======
...
##What type of vst stabilisation model is optimal?

```{r, message=FALSE,fig.height=15, fig.width=15, results=FALSE, fig.keep='all'}

beta_custom_norm_PCoA_elli <- function(ps, seed = 7888, normtype="vst", color="Repeats", design = "Repeats"){
  require(phyloseq)
  require(ggplot2)
  require(ggpubr)
  require(DESeq2)
  ...
  otus <- veganifyOTU(physeq)
  otus <- t(otus)
  deseq_counts_by_compl <- DESeqDataSetFromMatrix(otus, colData = meta, design = as.formula(paste( " ~ ", design)))
  deseq_counts_vst_by_compl <- varianceStabilizingTransformation(deseq_counts_by_compl)
  vst_trans_count_tab_complr <- assay(deseq_counts_vst_by_compl)
  otu.table.compl <- otu_table(vst_trans_count_tab_complr, taxa_are_rows = TRUE)
  otu.table.compl[otu.table.compl < 0.0] <- 0.0
  ps.varstab <- physeq
  otu_table(ps.varstab) <- otu.table.compl
...
beta_plot_function_result_medium_soil <- beta_custom_norm_PCoA_elli(ps.f, design = "Medium_or_Soil")
beta_plot_function_result_medium_soil_soil <- beta_custom_norm_PCoA_elli(ps.f, design = "Medium_or_Soil + Soil")
beta_plot_function_result_medium_repeats <- beta_custom_norm_PCoA_elli(ps.f, design = "Medium_or_Soil/Soil")

ggarrange(beta_plot_function_result_medium_soil[[1]], beta_plot_function_result_medium_soil_soil[[1]], beta_plot_function_result_medium_repeats[[1]], ncol = 1, nrow = 3, common.legend = TRUE, legend = "none", font.label = list(size = 12, face = "bold", color ="black"))
plot_scree(beta_plot_function_result[[4]])
ord_b <- beta_plot_function_result[2]
beta_plot_function_result_medium_soil[[1]]

```

##Kinda searching of meaning in diversity param histeric
Use with caution
>>>>>>> 0e52d7628c241006ce44743f0e759f68c27e184b
###g/bs core
```{r}
...
```{r}
ps.merged.repeats <- merge_samples(ps.f, "Description")
otus.merged  <- ps.merged.repeats@otu_table@`.Data`
otus.merged.m  <- as.matrix(t(otus.merged))
inext.merged  <- iNEXT(otus.merged.m, q=0, datatype="abundance")
```

```{r}
in.out <- iNEXT(otus.merged.m, q=c(0, 1, 2), datatype="abundance", endpoint=500)
```
...
```{r}
ggiNEXT(in.out, type=2, color.var="site") +
  ylim(c(0.9,1)) +
